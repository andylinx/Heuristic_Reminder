"""Utilities for Llama Guard-based safety evaluation."""
...
# Constants
DEFAULT_MODEL_PATH = os.environ.get("LLAMA_GUARD_MODEL_PATH", "")
...
class LlamaGuard:
    ...
    def __init__(self, model_path: str = DEFAULT_MODEL_PATH):
        ...
        if not self.model_path:
            raise ValueError(
                "Llama Guard model path is required. Pass --guard-model-path or set LLAMA_GUARD_MODEL_PATH."
            )
        self.processor = None
        self.model = None
        self.device = None
    ...
    def _initialize_model(self):
        ...
        # transformers >=4.57 defaults Llama4 generate() to a StaticCache whose
        # sliding-window init crashes (max_cache_len=None). Force a dynamic cache
        # so short scoring generations work across transformers versions.
        try:
            self.model.generation_config.cache_implementation = None
        except Exception:
            pass
        # Llama-Guard-4's config has attention_chunk_size=None, but transformers
        # eagerly builds a chunked-attention mask and raises. All 48 layers are
        # full-attention (no_rope_layers all 1), so chunking is never really used;
        # set a huge chunk so chunked==full for any realistic input length.
        try:
            tc = getattr(self.model.config, "text_config", self.model.config)
            if getattr(tc, "attention_chunk_size", None) in (None, 0):
                tc.attention_chunk_size = 1 << 20
        except Exception:
            pass
    ...
    def get_unsafe_probability(self, text: str, image_path: Optional[str] = None) -> float:
        ...
        with torch.no_grad():
            # use_cache=False bypasses the Llama4 sliding-window cache-mask path
            # that crashes under transformers>=4.57 (sliding_window=None on some
            # layers). Only ~10 tokens are generated, so the cost is negligible.
            outputs = self.model.generate(
                **inputs,
                max_new_tokens=10,
                do_sample=False,
