#!/usr/bin/env python3
"""Lightweight client for vLLM OpenAI-compatible endpoints."""
...
class VLLMClient:
    ...
    def __init__(self, base_url: str = "http://localhost:8000"):
        ...
        self.client = OpenAI(
            api_key="EMPTY",  # pragma: allowlist secret -- local vLLM needs no real key
            base_url=f"{self.base_url}/v1"
        )

    def resolve_model(self, model: Optional[str] = None) -> str:
        """Resolve a concrete model id, auto-detecting from the server if needed."""
        if model:
            return model
        models = self.get_models()
        if not models:
            raise RuntimeError("No models available from the vLLM server")
        return models[0]["id"]
